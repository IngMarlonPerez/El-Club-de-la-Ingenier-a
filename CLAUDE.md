...
- **Asistente de IA:** `pages/api/chat.js` — proxy server-side con **tres proveedores** en cascada: Groq (primario, `openai/gpt-oss-20b` con `reasoning_effort:"low"`) → OpenRouter (`openrouter/free`, auto-router) → NVIDIA NIM (`meta/llama-3.1-8b-instruct`). El presupuesto es un contador global por proveedor (no por usuario) guardado en `public.ia_uso_diario` (Supabase, migración `0002_ia_uso_diario.sql`, incrementado vía la función `increment_ia_uso`) — se eligió global en vez de por-IP porque muchos usuarios comparten la wifi de la universidad. Si los tres proveedores agotan su cuota del día (u OpenRouter, el más limitado con 45/día en el free tier sin fondear, es el único disponible y también se agota), el bot responde pidiendo volver mañana en vez de fallar. Mantiene además el rate limit corto en memoria por IP (anti-flood) y la validación con Zod. El system prompt incluye Misión/Visión/Quiénes Somos, un par de chistes de ingeniería (uso opcional) y los enlaces de la comunidad (GitHub, grupo de Facebook). El widget de chat vive en `public/index.html` y solo habla con `/api/chat`, nunca directo a los proveedores. **Nota (ago/2026):** `llama-3.1-8b-instant` (Groq) y `nvidia/llama-3.1-nemotron-70b-instruct` (NVIDIA) dejaron de estar disponibles (404 confirmado contra las APIs reales, no falta de cuota) y se reemplazaron por los modelos de arriba — si el asistente vuelve a fallar, primero descartar modelos retirados antes de asumir que se acabaron los créditos (`GET /v1/models` de cada proveedor con la API key real confirma qué sigue vigente).
...
- **SEO:** `<title>`/meta description reales, Open Graph, canonical, `public/sitemap.xml` y `public/robots.txt` apuntando al dominio real en producción (`elclubdelaingenieria.dpdns.org` — dominio temporal gratuito; el `.com` sigue siendo el dominio objetivo cuando se compre, ver sección 3). Un solo `<h1>` en toda la página ("El Club de la Ingeniería"), jerarquía H2/H3 consistente por sección.
- **Misión, Visión y sección de Eventos:** agregadas en `#about` (tarjetas Misión/Visión) y en la nueva sección `#eventos` (actividades recurrentes + CTA hacia `#join`). Enlaces de comunidad (grupo de Facebook, repo de GitHub) sumados a `#media`.
- **Backend más profesional (ago/2026):** rate limiting real vía `lib/rateLimit.js` (función atómica `increment_rate_limit` en Supabase, migración `0004_rate_limits.sql`, con respaldo en memoria si Supabase no está configurado) — reemplaza los `Map` en memoria que antes vivían duplicados en `pages/api/chat.js`, `pages/api/join.js` y `lib/jobs/rateLimit.js` (ese archivo ya no existe). Monitoreo de errores con Sentry (`@sentry/nextjs`, `sentry.server.config.js`/`sentry.client.config.js`, `next.config.js` envuelto con `withSentryConfig`) — opt-in, no hace nada sin `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN`. Dockerización (`Dockerfile`, `docker-compose.yml`, `next.config.js` con `output: 'standalone'`) solo para desarrollo local portable — la producción real sigue en Vercel sin cambios (cron, dominio, SSL). Todas las 5 tablas de Supabase (`miembros`, `solicitudes`, `ia_uso_diario`, `job_listings_cache`, `rate_limits`) confirmadas aplicadas y en uso en el proyecto real — la línea de "Pendiente aplicar migraciones" que estaba aquí ya no aplica.
- **OAuth (ago/2026):** confirmado en vivo contra `/auth/v1/settings` de Supabase que Google, GitHub y Facebook están los 3 habilitados y activos (Facebook ya pasó la revisión de Meta) — el login social funciona en producción. Los tres botones ya estaban implementados en `public/index.html` desde antes.
- **Guardar CV subido (ago/2026):** capacidad opt-in, requiere login — migración `0005_cv_uploads.sql` (bucket privado `cv-uploads` en Supabase Storage + tabla `cv_uploads`, RLS por `auth.uid()`, mismo patrón que `miembros`). El navegador del usuario logueado sube el PDF directo a Storage, sin pasar por el servidor. Sin login, el analizador de CV sigue funcionando exactamente igual que antes (transitorio, sin guardar nada). `pages/proyectos/buscador-empleo.js` ahora inicializa su propio cliente de Supabase (`@supabase/supabase-js`, bundleado por Next.js — no esm.sh, eso es solo para `public/index.html` que no tiene bundler).
- **Lighthouse (ago/2026):** dos hallazgos reales corregidos — `.btn` no declaraba `background` (texto casi blanco sobre el gris por defecto del navegador, invisible) y el import de `@supabase/supabase-js` en `public/index.html` vía esm.sh generaba ~17 solicitudes en cadena, reducido a 7 con `?bundle`. El monorepo/subdominios NO tiene relación con estos hallazgos de rendimiento — es una decisión de arquitectura aparte, sin apuro (el usuario solo tiene un dominio gratis por ahora, sin presupuesto para más hasta que el club genere ingresos).
- **Pendiente (no implementado aún):** correr la migración `0005_cv_uploads.sql` en el SQL Editor de Supabase (mismo flujo manual de siempre); monorepo con `apps/templates/`, subdominios, CI/CD en GitHub Actions — pospuesto, sin necesidad real hoy; explorar IA local con GPU (Ollama u otro) — decisión aparte, pospuesta a propósito.
