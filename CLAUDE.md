...
- **Pendiente (no implementado aún):** correr la migración `0004_rate_limits.sql` en el SQL Editor de Supabase (mismo flujo manual de siempre); configurar las apps OAuth de Google/GitHub en sus consolas y pegar las credenciales en Supabase; Facebook OAuth (pendiente de revisión de Meta); monorepo con `apps/templates/`, subdominios, CI/CD en GitHub Actions; guardar los PDF de CV subidos en el Buscador de Empleo (requiere primero actualizar el consentimiento/política de privacidad — decisión deliberadamente pospuesta); explorar IA local con GPU (Ollama u otro) — decisión aparte, pospuesta a propósito.
