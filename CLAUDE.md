...
- **Asistente de IA:** `pages/api/chat.js` — proxy server-side con **dos proveedores**: Groq (primario, `llama-3.1-8b-instant`) con respaldo automático a NVIDIA NIM (`nvidia/llama-3.1-nemotron-70b-instruct`) cuando Groq falla o agota su presupuesto diario. El presupuesto es un contador global por proveedor (no por usuario) guardado en `public.ia_uso_diario` (Supabase, migración `0002_ia_uso_diario.sql`, incrementado vía la función `increment_ia_uso`) — se eligió global en vez de por-IP porque muchos usuarios comparten la wifi de la universidad. Si ambos proveedores agotan su cuota del día, el bot responde pidiendo volver mañana en vez de fallar. Mantiene además el rate limit corto en memoria por IP (anti-flood) y la validación con Zod. El system prompt incluye Misión/Visión/Quiénes Somos, un par de chistes de ingeniería (uso opcional) y los enlaces de la comunidad (GitHub, grupo de Facebook). El widget de chat vive en `public/index.html` y solo habla con `/api/chat`, nunca directo a los proveedores.
